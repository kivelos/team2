...
public class InterviewController {
  ...
  public ModelAndView editCandidate(@PathVariable long id, HttpServletRequest request, HttpServletResponse response) {
    logging.runMe(request);
    ModelAndView modelAndView = new ModelAndView("redirect:" + "/interviews/" + id);
    try (Connection dbConn = ConnectorDB.getConnection()) {
      String s_planDate = request.getParameter("planDate").trim();
      String s_factDate = request.getParameter("factDate").trim();
      String s_id_candidate = request.getParameter("id_candidate").trim();
  ...
  public ModelAndView getFilteredEntities(HttpServletRequest request) {
    ...
    try (Connection dbConn = ConnectorDB.getConnection()) {
      ...
      String s_id_vacancy = request.getParameter("id_vacancy").trim();
      InterviewDao interviewDao = new InterviewDao(dbConn);
      List<Interview> interviews = interviewDao.getFilteredEntitiesPage(s_planDate, s_factDate, s_id_candidate, s_id_vacancy);
      modelAndView.addObject("interviews_list", interviews);
    } catch (SQLException e) {
      e.printStackTrace();
    }
